package com.b07.bankofjarm.database;
...
public class DatabaseDriverA extends SQLiteOpenHelper {
  ...
  protected String getMessageViewState(int messageId) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
    Cursor cursor = sqLiteDatabase.rawQuery("SELECT VIEWED FROM USERMESSAGES WHERE ID = ?",
        new String[]{String.valueOf(messageId)});
    cursor.moveToFirst();
    String result = cursor.getString(cursor.getColumnIndex("VIEWED"));
    cursor.close();
    return result;
  }
