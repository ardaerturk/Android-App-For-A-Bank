...
public class DatabaseSelector {
  ...
  /**
   * return the ID's of the account's users.
   * @param accountId the id of the account.
   * @param connection the connection to the database.
   * @return a result set containing the users of the account.
   * @throws SQLException thrown when there is a connection problem.
   */
  protected static ResultSet getUserIds(int accountId, Connection connection) throws SQLException {
    String sql = "SELECT USERID FROM USERACCOUNT WHERE ACCOUNTID = ?";
    PreparedStatement preparedStatement = connection.prepareStatement(sql);
  ...
  /**
   * 
   * @param messageId is the ID of the message whose status you wish to view.
   * @param connection connection to the database.
   * @return the viewed state from the database as a string.
   * @throws SQLException if there is a problem connecting to the database.
   */
  protected static String getMessageViewedState(int messageId, Connection connection) 
      throws SQLException {
    String sql = "SELECT VIEWED FROM USERMESSAGES WHERE ID = ?";
    PreparedStatement preparedStatement = connection.prepareStatement(sql);
    preparedStatement.setInt(1, messageId);
    return preparedStatement.executeQuery().getString("VIEWED");
  }
}
